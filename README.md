...
- 地下盾构/综合管廊穿越既有地下区间时，按工程师模板生成全站仪平面变形 + 静力水准沉降联合快报。
- 预警、报警、红色预警、消警申请、专题分析和项目总结。

当前自动化脚本重点支持三类穿越期高频报表：全站仪/测量机器人平差成果快报、上海华桓静力水准沉降快报、地下盾构穿越既有地下区间的联合快报模板。人工监测和其他设备可以按 skill 内的资料规则继续扩展。
...
```bash
npm install -g github:railwise-cn/di-bao-monitoring-skill#v0.2.2
di-bao-monitoring-skill install --force
```
...
当用户提到“通途西路模板”“地下盾构穿地下线”“全站仪平面变形 + 静力水准沉降联合快报”等任务时，skill 会优先按工程师 workbook 模板保持版式：`前台` 双线上/下行快报、`前台 (静力水准)` 沉降页、`Z1 全 (2)`/`Z1 （静力水准)` 数据页、累计曲线图和隐藏辅助页，而不是套用高架桥墩 `MCC/MCW/MCQX` 倾斜模板。
