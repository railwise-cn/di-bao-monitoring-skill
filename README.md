...
- 地下盾构/综合管廊穿越既有地下区间时，按工程师模板建立全站仪平面变形 + 静力水准沉降联合快报的数据源和展示映射。
- 预警、报警、红色预警、消警申请、专题分析和项目总结。

当前自动化脚本重点支持两类穿越期高频报表生成：全站仪/测量机器人平差成果快报、上海华桓静力水准沉降快报；并补充了地下盾构穿越既有地下区间联合快报的模板识别、输入契约、数据来源和展示映射。人工监测和其他设备可以按 skill 内的资料规则继续扩展。
...
```bash
npm install -g github:railwise-cn/di-bao-monitoring-skill#v0.2.3
di-bao-monitoring-skill install --force
```
...
当用户提到“通途西路模板”“地下盾构穿地下线”“全站仪平面变形 + 静力水准沉降联合快报”等任务时，skill 会优先按工程师 workbook 模板保持版式：`前台` 双线上/下行快报、`前台 (静力水准)` 沉降页、`Z1 全 (2)`/`Z1 （静力水准)` 数据页、累计曲线图和隐藏辅助页，而不是套用高架桥墩 `MCC/MCW/MCQX` 倾斜模板。当前版本先提供数据源、输入契约和模板检查层；完整自动填报生成器应在实例验证后继续补齐。
...
通途西路类项目和绕城高速高架桥墩项目不是同一种模板。前者是地下盾构/综合管廊穿越既有地下区间，通常由全站仪出平面变形，由静力水准出沉降，报表应与工程师 workbook 保持一致。

典型工作簿结构：

| 工作表 | 用途 |
|---|---|
| `前台` | 上行线/下行线双列快报，含工程概况、施工进度、全站仪数据分析、最大值和结论 |
| `前台 (静力水准)` | 静力水准结构沉降页，含本次变化、穿越期间累计、本次最大和结论 |
| `Z1 全 (2)` | 全站仪/盾构累计数据页，按时间列存放道床沉降、结构沉降、水平位移、水平收敛 |
| `Z1 （静力水准)` | 静力水准沉降时序数据页 |
| `盾构总累计曲线图`、`日变量曲线图` | 曲线图和辅助数据 |
...
- 静力水准平台/API 沉降成果。
- 用户最新工程师 Excel 模板。
- 当前施工工况、报表截止时间、上期时间、点名映射/分区和影响范围图片。

公开仓库不会内置用户的项目原始 workbook；工程师模板应作为本地运行时模板传入，避免泄露项目资料。

### 联合报表数据来源和展示方式

地下盾构穿越地下线的联合报表先分清“谁负责什么数据”，再填 workbook：

| 监测内容 | 默认来源 | 落表位置 | 前台展示 |
|---|---|---|---|
| 沉降/高程变化 | 上海华桓静力水准 `curOffset/totalOffset`，或项目确认的静力水准成果 | `Z1 （静力水准)` | `前台 (静力水准)`，必要时回填 `前台` 沉降汇总 |
| 水平位移 | 全站仪平台平差后坐标差，常见东西向看 `Y`、南北向看 `X` | `Z1 全 (2)` | `前台` 上行线/下行线水平位移区 |
| 水平收敛 | 全站仪成对点或工程师模板既有公式 | `Z1 全 (2)` 或隐藏辅助页 | `前台` 水平收敛区和曲线 |
| 工况和图片 | 用户/工程师确认的现场工况、影响范围图、布点图 | `前台`、`前台 (静力水准)` | 工程概况、施工进度、图片区和结论 |

推荐先复制并填写：

```text
~/.codex/skills/di-bao-monitoring/assets/underground-shield-combined-input-template.json
```

这个输入契约用于确认报表截止时间、上期时间、全站仪 CSV、静力水准 CSV、点名/线别/环号/影响区映射、图片和 workbook sheet 映射。它不保存账号、密码或 Cookie。

拿到新的工程师模板后，可先检查结构：

```bash
python3 ~/.codex/skills/di-bao-monitoring/scripts/inspect_underground_shield_template.py \
  "{{工程师模板.xlsx}}" \
  --output "模板结构核对.md"
```

检查结果会列出工作表显隐、尺寸、打印区域、图片、图表、关键文本和公式样本，便于后续与工程师模板逐项对照。
...
| `inspect_underground_shield_template.py` | 检查地下盾构联合报表工程师模板结构，输出 sheet、打印区域、图片、图表、关键文本和公式样本 |
...
gh release create v0.2.2 --title "v0.2.2" --notes "更新说明"
```
